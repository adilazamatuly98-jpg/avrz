...
USERS = {
    ...
    "nachalnik":       {"password": "1", "name": "Начальник цеха",              "role": "nachalnik",      "uchastok": None,          "cex": None},
    # ── ВРЦ ──
    ...
    "defekt_vrc":      {"password": "1", "name": "Дефектоскопист · ВРЦ",        "role": "defektoskopist", "uchastok": "vrc_def",     "cex": "vrc"},
    # ── КТЦ ──
    ...
    "defekt_ktc":      {"password": "1", "name": "Дефектоскопист · КТЦ",        "role": "defektoskopist", "uchastok": "kolesny",     "cex": "ktc"},
    # ── Обратная совместимость (старые логины) ──
    "defekt1":         {"password": "1", "name": "Дефектоскопист (Колёсный)",   "role": "defektoskopist", "uchastok": "kolesny",     "cex": "ktc"},
    "defekt2":         {"password": "1", "name": "Дефектоскопист (Вагонный)",   "role": "defektoskopist", "uchastok": "vrc_def",     "cex": "vrc"},
}
...
def _seed():
    ...
    return {
        "counters": {"wagons": 3, "opzs": 12, "nakatka": 1,
...
def api_opzs():
    ...
    # Мастер роликовый — видит только Роликовый ОПЗС (1 документ на смену)
    elif u["role"] == "master_roller" and not uch:
        docs = [d for d in docs if d["uchastok"] == "roller"]
    # Дефектоскопист ВРЦ — ОПЗС по Автосцепке И Тележечному
    elif u["role"] == "defektoskopist" and u.get("uchastok") in ("vrc_def", "wagon_def") and not uch:
        docs = [d for d in docs if d["uchastok"] in ("avtoscepka", "telega")]
    # Дефектоскопист КТЦ (Колёсный) — ОПЗС не ведёт
    elif u["role"] == "defektoskopist" and u.get("uchastok") == "kolesny" and not uch:
        docs = []
    return jsonify(docs)
